...
add_periods <- function(init_data, 
                        period="long", 
                        gap=FALSE, 
                        year_chosen=FALSE){
  if (is.list(year_chosen)==TRUE){
    y_threshold_low <- year_chosen[["old_low"]]
    y_threshold_middle <- year_chosen[["old_up"]]
    y_threshold_up <- year_chosen[["new_low"]]
    y_threshold_highest <- year_chosen[["new_up"]]
    
    data_work <- data.table(init_data)
    data_work <- data_work[year > y_threshold_low]
    data_work <- data_work[year <= y_threshold_highest]
    data_work <- data_work %>%
...
make_plot_data <- function(raw_data, period_considered="long", 
                           gap_used=FALSE, chosen_list=FALSE) {
  stopifnot(period_considered %in% c("long", "crisis"))
  if (is.list(chosen_list)==FALSE){
    work_data_p <- add_periods(raw_data, period = period_considered, 
                               gap = gap_used) 
  } else {
    work_data_p <- add_periods(raw_data, period = period_considered, 
                               gap = gap_used, year_chosen = chosen_list) 
  }
  # browser()
  ...
  # browser()
  print(1)
  if (is.list(chosen_list)==TRUE){
    final_share_years <- chosen_list[["new_up"]] - 2
  } else {
    final_share_years <- 2012
  }
  late_exp_share_reg <- work_data_p %>%
    filter(year>final_share_years) %>%
  ...
  diff_data_diffs <- work_data %>%
    select(exporter, period, commoditycode, exp_val_total, 
           exp_share_wt_mean, exp_share_reg_mean) %>%
  ...
  data_final <- left_join(fulltime_data, diff_data_diffs, 
                          by=c("exporter", "commoditycode")) %>%
  ...
  var_label(data_final) <- list(
    ...
    av_pci_w = "Weighted average complexity of the product over the full time 
    considered. Weights are yearly total exports, so it differs among countries.", 
    av_exp_share_reg = "Average export share of the product 
    in it's regions' total exports.", 
    late_exp_share_reg = "Average export share of the product 
    in it's regions' total exports for the last 3 years of the sample.", 
    diff_exp_share_reg = "Difference between average export share 
    in region's total exports in early and late period.", 
    diff_exp_share_wt = "Difference between average export share 
    in world total exports in early and late period.", 
    diff_exp_val_total = "Difference between the sums of export value 
    in early and late period.",
    ratio_exp_share_reg = "Ratio of average export share in region's total exports 
    in early and late period (late/early).",
    ratio_exp_share_wt = "Ratio of average export share in world total exports 
    in early and late period (late/early).",
    ratio_exp_val_total = "Ratio of the sums of export value 
    in early and late period (late/early)."
    )
  return(data_final)
}
...
make_single_cntry_plot <- function(cntry, period="long", 
  ...
  if (log_y_trans==TRUE){
    ret_list[["pos_plot"]] <- expl_plot_pos + 
      scale_y_continuous(trans='log10') + ylab(paste0("log(", y_val, ")"))
  ...
  if (log_x_trans==TRUE){
    ret_list[["pos_plot"]] <- ret_list[["pos_plot"]] + 
      scale_x_continuous(trans='log10') + xlab(paste0("log(", x_val, ")"))
    ret_list[["neg_plot"]] <- ret_list[["neg_plot"]] + 
      scale_x_continuous(trans='log10') + xlab(paste0("log(", x_val, ")"))
  }
  
  
  ret_list[["pos_plot"]] <- ret_list[["pos_plot"]] + 
    geom_smooth(method = method_geom_smooth)
  ret_list[["neg_plot"]] <- ret_list[["neg_plot"]] + 
    geom_smooth(method = method_geom_smooth)
...
make_wls_plot <- function(regression_data, # nimmt long or crisis data
  ...
  exp_plot_re_coef <- ggplot(reg_dat_used,   
    ...
    geom_line(color='#00394d', data = predicted_df, 
              aes(x=complexity, y=fit), linetype="solid") +
    geom_line(color='#0086b3', data = predicted_df, 
              aes(x=complexity, y=upr), linetype="dashed") +
    geom_line(color='#0086b3', data = predicted_df, 
              aes(x=complexity, y=lwr), linetype="dashed") +
    ggtitle(
      paste0(country_name, 
...
set_up_reg_formula <- function(dependent_variable, independent_variables){
  ...
  for(i in 1:length(independent_variables)) {
    regression_formula <- paste0(
      regression_formula, independent_variables[i], " + ")
    }
...
get_dist_changes <- function(raw_dat, # Either long_data or crisis_data
  ...
  if (substr(var_of_interest, nchar(var_of_interest)-2, 
             nchar(var_of_interest)) == "log"){
  ...
  stats_list[["positive_v"]] <- sum(filter(work_data_v1, 
                                           UQ(as.name(var_con))>0)[var_con]) # The amount of positive changes
  stats_list[["negative_v"]] <- sum(filter(work_data_v1, 
                                           UQ(as.name(var_con))<0)[var_con]) # The amount of negative changes
  stats_list[["positive_n"]] <- sum(!is.na(filter(work_data_v1, 
                                                  UQ(as.name(var_con))>0)[var_con])
                                    )  # The number of positive changes
  stats_list[["negative_n"]] <- sum(!is.na(filter(work_data_v1, 
                                                  UQ(as.name(var_con))<0)[var_con])
                                    )  # The number of negative changes
...
make_reg_frame <- function(regression_data, 
  ...
  #browser()
  if (period_cons=="long"){
    dist_changes_frame <- get_dist_changes(
      reg_dat_raw, countries_cons[1], 
      period_cons = period_cons, 
      gap = gap_cons, 
      var_of_interest = dep_var) 
    for (i in 2:length(countries_cons)){
      dist_changes_frame <- rbind(
        dist_changes_frame, get_dist_changes(
          reg_dat_raw, countries_cons[i], 
          period_cons = period_cons, 
          gap = gap_cons, 
          var_of_interest = dep_var)) 
    }
  ...
  final_frame <- left_join(data.frame(reg_list), 
                           dist_changes_frame, by=c("country", "period")
                           )
